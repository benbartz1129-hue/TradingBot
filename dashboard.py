...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .tab { flex: 1; text-align: center; padding: 8px; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; color: #8e8e93; text-decoration: none; display: block; }
    ...
    .win { color: #34c759; }
    .loss { color: #ff3b30; }
    input[type=password] { width: 100%; padding: 12px; border: 1px solid #c6c6c8; border-radius: 10px; font-size: 16px; margin: 8px 0; }
...
<body>
  ...
  <div class="login-card">
    ...
    <form method="POST" action="/login">
      ...
      <button class="approve" type="submit" style="width:100%; margin-top:8px; padding:14px; border:none; border-radius:10px; font-size:16px; font-weight:600; cursor:pointer;">Login</button>
    </form>
  </div>
  ...
  <div class="tabs">
    ...
    <a class="tab {% if tab == 'history' %}active{% endif %}" href="/?tab=history">
      History
    </a>
  </div>
    ...
    {% for trade_id, trade in trades.items() %}
      {% if trade.status == 'pending' %}
        ...
        <div class="card">
          ...
          <div style="margin-top:6px;">
            ...
            &nbsp;{{ "%.4f"|format(trade.quantity|float) }} shares @ ~${{ "%.2f"|format(trade.price|float) }}
          </div>
          <div class="detail">Est. Value: ${{ "%.2f"|format(trade.estimated_value|float) }}</div>
      ...
      {% elif trade.status == 'approved' %}
        ...
        <div class="card">
          ...
          <div class="detail">{{ trade.side.upper() }} {{ "%.4f"|format(trade.quantity|float) }} shares</div>
          <div style="color:#34c759; margin-top:8px; font-weight:600;">✅ Executing...</div>
        </div>
      {% endif %}
    {% endfor %}
    {% if shown|length == 0 %}
    ...
    {% if history %}
      ...
      <div class="card">
        ...
        <div style="margin-top:6px;">
          ...
          {% if entry.quantity %}
          &nbsp;{{ "%.4f"|format(entry.quantity|float) }} shares @ ~${{ "%.2f"|format(entry.price|float) }}
          {% endif %}
        </div>
        <div class="detail">Est. Value: ${{ "%.2f"|format(entry.estimated_value|float) }}</div>
  ...
  <div class="footer">
    <div class="card">
      <div class="detail">Account ••••8850 | Next scans: 8:30am · 11:00am · 2:30pm CT</div>
    </div>
  </div>
